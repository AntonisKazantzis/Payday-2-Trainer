...
if inGame() then
    ...
    if not mymenu then
        mymenu = mymenu or SimpleMenu:new("Payday 2 Trainer By AntonisK", "In Game Main Options", in_game)
    end

    mymenu:show()
else
	p2t_require("Trainer/menu/pre-game/player_progress")
	p2t_require("Trainer/menu/pre-game/contract")
	p2t_require("Trainer/menu/pre-game/unlock_all")
	p2t_require("Trainer/menu/pre-game/safehouse")
	p2t_require("Trainer/menu/pre-game/anti_cheat")
    ...
    pre_game = pre_game or 
	{
		...
		{ text = "Anti Cheat Options", callback = call_anti_cheat }, 
    ...
    if not mymenu then
        mymenu = mymenu or SimpleMenu:new("Payday 2 Trainer By AntonisK", "Pre Game Main Options", pre_game)
    end
