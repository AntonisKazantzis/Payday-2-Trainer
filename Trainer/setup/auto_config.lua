...
--[[
    -- Anti Cheat
    p2t.anti_anti_hidden_modlist,             -- Anti Anti Hidden Modlist
    p2t.dont_publish_statictics,              -- Dont Publish Statistics To Steam
    p2t.hide_modlist,                         -- Hide ModList
    p2t.hide_trainer,                         -- Hide Trainer From Modlist
    p2t.no_chat_spam,                         -- No Chat Spam For Cheating
    p2t.no_verify_content,                    -- No Verify Content Of Player
    p2t.spoof_actions,                        -- Spoof Player Interaction
    p2t.spoof_inventory                       -- Spoof Player Inventory
    p2t.spoof_pd2_community                   -- Spoof Internal Calls When Kicking For P2D Community
    ...
    p2t.config.longer_camera_loop,            -- Longer Camera Loop Duration
    p2t.config.no_alarm_laser,                -- Removes Non-Static Alarm Lasers
    p2t.config.no_pagers,                     -- Bodies Doesn't Have Pagers
    p2t.config.no_pager_on_domination,        -- No Pager On Dominated Enemies
    ...
    p2t.config.no_concussion_tinnitus,        -- Removes Concussion Tinnitus From Flashbang
    ...
    p2t.config.move_while_interacting,        -- Able To Interact While Interacting
    ...
    p2t.config.shoot_while_interacting,       -- Able To Shoot While Interacting
    ...
    p2t.config.infinite_converts,             -- Infinite Amount Of Convertions
    ...
    -- Player Weapon
    p2t.config.bow_tweaks,                    -- Bow And Arrow Tweaks
    ...
    p2t.config.melee_tweaks,                  -- Melee Tweaks
    p2t.config.no_ads_movement,               -- No Aditional Ads Movement
    p2t.config.no_ads_recoil,                 -- No Ads Recoil
    ...
    -- Pre Planning
    p2t.config.all_crew_boosts,               -- All Crew Boosts Active
    p2t.config.infinite_drawing_points,       -- Infinite Drawing Points
    p2t.config.infinite_favors,               -- Infinite Favors
--]]

-- Anti Cheat
if p2t_config.anti_anti_hidden_modlist then
    p2t_require("Trainer/addons/anti-cheat/anti_anti_hidden_modlist")
end

if p2t_config.dont_publish_statistics then
    p2t_require("Trainer/addons/anti-cheat/dont_publish_statistics")
end

if p2t_config.hide_modlist then
    p2t_require("Trainer/addons/anti-cheat/hide_modlist")
end

if p2t_config.hide_trainer then
    p2t_require("Trainer/addons/anti-cheat/hide_trainer")
end

if p2t_config.no_chat_spam then
    p2t_require("Trainer/addons/anti-cheat/no_chat_spam")
end

if p2t_config.no_verify_content then
    p2t_require("Trainer/addons/anti-cheat/no_verify_content")
end

if p2t_config.spoof_actions then
    p2t_require("Trainer/addons/anti-cheat/spoof_actions")
end

if p2t_config.spoof_inventory then
    p2t_require("Trainer/addons/anti-cheat/spoof_inventory")
end

if p2t_config.spoof_pd2_community then
    p2t_require("Trainer/addons/anti-cheat/spoof_pd2_community")
end

-- Contract
if p2t_config.free_job_purchases then
    p2t_require("Trainer/addons/contract/free_job_purchases")
end

if p2t_config.complete_all_side_jobs then
    p2t_require("Trainer/addons/contract/complete_all_side_jobs")
end
...
if p2t_config.longer_camera_loop then
    p2t_require("Trainer/addons/heist/longer_camera_loop")
end
...
if p2t_config.no_pager_on_domination then
    p2t_require("Trainer/addons/heist/no_pager_on_domination")
end
...
if p2t_config.no_concussion_tinnitus then
    p2t_require("Trainer/addons/player/no_concussion_tinnitus")
end
...
if p2t_config.no_explosion_shake then
    p2t_require("Trainer/addons/player/no_explosion_shake")
end
...
if p2t_config.move_while_interacting then
    p2t_require("Trainer/addons/player-interaction/move_while_interacting")
end

if p2t_config.dont_interrupt_on_distance then
    p2t_require("Trainer/addons/player-interaction/dont_interrupt_on_distance")
end
...
if p2t_config.shoot_while_interacting then
    p2t_require("Trainer/addons/player-interaction/shoot_while_interacting")
end
...
if p2t_config.infinite_converts then
...
-- Player Weapon
if p2t_config.bow_tweaks then
    p2t_require("Trainer/addons/player-weapon/bow_tweaks")
end
...
if p2t_config.melee_tweaks then
    p2t_require("Trainer/addons/player-weapon/melee_tweaks")
end

if p2t_config.no_ads_movement then
    p2t_require("Trainer/addons/player-weapon/no_ads_movement")
end

if p2t_config.no_ads_recoil then
    p2t_require("Trainer/addons/player-weapon/no_ads_recoil")
end
...
if p2t_config.shoot_through_anything then
    p2t_require("Trainer/addons/player-weapon/shoot_through_anything")
end

-- Pre Planning
if p2t_config.all_crew_boosts then
    p2t_require("Trainer/addons/pre-planning/all_crew_boosts")
end
