...
grab_all_small_loot = grab_all_small_loot or function()
	function grabsmallloot()
		interactbytweak("cut_glass","crate_loot","crate_loot_crowbar","safe_loot_pickup","diamond_pickup","tiara_pickup","money_wrap_single_bundle","invisible_interaction_open","mus_pku_artifact")
	end
	grabsmallloot()
...
grab_all_big_loot = grab_all_big_loot or function()
	function graballbigloot()
		interactbytweak("trai_printing_plates_carry","diamond_pickup","red_diamond_pickup","diamonds_pickup_full","diamonds_pickup","shape_charge_plantable","gen_pku_cocaine_pure","money_small","money_scanner","money_luggage","money_wrap","money_bag","corpse_dispose","hold_pku_drk_bomb_part","hold_take_server","weapon","ammo","painting","old_wine","ordinary_wine","drk_bomb_part","evidence_bag","coke","coke_pure","diamond_necklace","diamonds","artifact_statue","prototype","yayo","meth_half","samurai_armor","turret","roman_armor","samurai_suit","weapons","carry_drop","painting_carry_drop","money_wrap","gen_pku_jewelry","taking_meth","gen_pku_cocaine","take_weapons","gold_pile","hold_take_painting","invisible_interaction_open","gen_pku_artifact","gen_pku_artifact_statue","gen_pku_artifact_painting")
	end
	graballbigloot()
...
open_all_doors = open_all_doors or function()
	function openalldoors()
		interactbytweak("cas_open_door","open_door_with_keys","requires_ecm_jammer_double","requires_ecm_jammer","cas_security_door","cas_open_securityroom_door","pick_lock_easy_no_skill", "pick_lock_hard_no_skill", "pick_lock_hard", "open_from_inside", "open_train_cargo_door")
	end
	openalldoors()
...
toggle_prevents = toggle_prevents or function(info)
	...
	if togglePrevents then
		...
		if not _actionRequest then _actionRequest = CopMovement.action_request end
		function CopMovement:action_request( action_desc )
			if action_desc.variant == "run" then return true end
			return _actionRequest(self, action_desc)
		end
...
toggle_remove_weapon_recoil = toggle_remove_weapon_recoil or function(info)
	...
	if toggleFireRate then
		...
		function NewRaycastWeaponBase:recoil_multiplier() return 1 end
...
toggle_increased_weapon_fire_rate = toggle_increased_weapon_fire_rate or function(info)
	toggleFireRate = not toggleFireRate
	if toggleFireRate then
		function NewRaycastWeaponBase:fire_rate_multiplier() return 100 end
...
toggle_increased_weapon_swap_speed = toggle_increased_weapon_swap_speed or function(info)
	...
	if toggleFireRate then
		...
		function PlayerStandard:_get_swap_speed_multiplier() return 1 end
...
toggle_instant_deploy = toggle_instant_deploy or function(info)
	...
	if toggleInstantDeploy then
		...
		function PlayerManager:selected_equipment_deploy_timer() return 2 end
...
toggle_infinite_bodybags = toggle_infinite_bodybags or function(info)
	...
	if toggleInfiniteBodyBags then
		...
		function PlayerManager:on_used_body_bag() self:_set_body_bags_amount(self._local_player_body_bags - 1) end
